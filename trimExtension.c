...
Trim extension
Rewrite the Trim program to also replace each string of spaces by a single space anywhere in the string
...
void delTooMuchSpace(const char src[], char dest[]);
...
int main()
{
    ...
    // Trim operation
    delTooMuchSpace(copyS, trimedS);
    delTooMuchSpace(copyS2, trimedS2);
...
// Function that remove trailing-leading space and replace each string of spaces by a single space
void delTooMuchSpace(const char src[], char dest[])
{
